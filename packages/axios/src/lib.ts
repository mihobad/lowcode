...
export async function fetch<R>(
  ...
  const toastPendingOptions: ToastOptions | undefined =
    toastPending === true
      ? { type: 'mask', once: true, duration: 0, forbidClick: true }
      : typeof toastPending === 'object'
        ? { type: 'mask', once: true, forbidClick: true, duration: toastPending.duration ?? 0, ...toastPending }
        : undefined
  ...
  try {
    const data = await handleResponseCommon(
      await axios.request<ResponseData<R> & ResponseException>({ ...axiosConfig, url, method }),
